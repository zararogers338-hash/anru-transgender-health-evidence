...
function cleanCitationUrl(value) {
  try {
    ...
    return url.protocol === "https:" ? url.toString().slice(0, 800) : "";
  } catch {
    return "";
  }
}
...
function normalizeEvidence(value) {
  ...
  return {
    ...
    results: Array.isArray(value.results) ? value.results.slice(0, 10).map((item) => ({
      work_id: cleanText(item?.work_id, 300),
      ...
      abstract_redistributable: item?.abstract_redistributable === true,
      license_url: cleanCitationUrl(item?.license_url),
      retrieved_at: cleanText(item?.retrieved_at, 64),
      citation_url: cleanCitationUrl(item?.citation_url),
      sources: Array.isArray(item?.sources) ? item.sources.slice(0, 8).map((source) => cleanText(source, 40)) : [],
    })) : [],
  };
}
...
class HistoryStore {
  ...
  preserveUnreadableFile() {
    if (this.recoveryFile || !fs.existsSync(this.file)) return;
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    const recovery = `${this.file}.unreadable-${stamp}.bak`;
    fs.copyFileSync(this.file, recovery);
    this.recoveryFile = recovery;
  }

  readAll() {
    try {
      const stats = fs.statSync(this.file);
      if (stats.size > MAX_HISTORY_BYTES) throw new Error("history file exceeds size limit");
      const value = JSON.parse(this.decode(fs.readFileSync(this.file)));
      if (!value || value.version !== 1 || !Array.isArray(value.sessions)) throw new Error("unsupported history format");
      return value.sessions.map((session) => this.normalizeSession(session)).filter(Boolean).slice(0, MAX_SESSIONS);
    } catch (error) {
      if (error?.code === "ENOENT") return [];
      this.preserveUnreadableFile();
      return [];
    }
  }
