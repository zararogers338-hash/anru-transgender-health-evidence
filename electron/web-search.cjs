...
const USER_AGENT = "Anru/0.1 (+local Pi evidence client)";
const MAX_PAGE_BYTES = 12 * 1024 * 1024;
const MAX_PAGE_TEXT = 40000;

const RESERVED_NETWORKS = new net.BlockList();
for (const [address, prefix, family] of [
  ...
  ["2001:2::", 48, "ipv6"], ["2001:db8::", 32, "ipv6"], ["fc00::", 7, "ipv6"],
  ["fe80::", 10, "ipv6"], ["ff00::", 8, "ipv6"],
]) RESERVED_NETWORKS.addSubnet(address, prefix, family);
...
function isPrivateAddress(address) {
  ...
  if (net.isIPv4(value)) return RESERVED_NETWORKS.check(value, "ipv4");
  if (net.isIPv6(value)) {
    const mapped = value.match(/^::ffff:(?:(\d+\.\d+\.\d+\.\d+)|([0-9a-f]{1,4}):([0-9a-f]{1,4}))$/);
    if (mapped) {
      const ipv4 = mapped[1] || `${parseInt(mapped[2], 16) >> 8}.${parseInt(mapped[2], 16) & 255}.${parseInt(mapped[3], 16) >> 8}.${parseInt(mapped[3], 16) & 255}`;
      return isPrivateAddress(ipv4);
    }
    return RESERVED_NETWORKS.check(value, "ipv6");
  }
  return true;
}
...
async function assertPublicUrl(input) {
  ...
  if (net.isIP(literalHost) && isPrivateAddress(literalHost)) throw new Error("Private network destinations are blocked");
...
async function boundedBuffer(response, maxBytes = MAX_PAGE_BYTES) {
...
async function boundedText(response, maxBytes = MAX_PAGE_BYTES) {
  return (await boundedBuffer(response, maxBytes)).toString("utf8");
}
...
class WebSearchService {
  ...
  async open(value) {
    ...
    if (/application\/pdf/i.test(contentType) || /\.pdf(?:$|[?#])/i.test(url.toString())) {
      const pdf = require("pdf-parse");
      const parsed = await pdf(await boundedBuffer(response));
      const text = compactText(parsed.text).slice(0, MAX_PAGE_TEXT);
      if (!text) throw new Error("PDF was downloaded but no readable text could be extracted");
      return {
        title: compactText(parsed.info?.Title) || "PDF document",
        url: url.toString(),
        text: `PDF TEXT (${parsed.numpages || "?"} pages; extracted locally): ${text}`,
      };
    }
...
module.exports = { WebSearchService, assertPublicUrl, boundedBuffer, compactText, extractPage, isPrivateAddress, parseSearchHtml, publicFetch };
