...
function withStore(run) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "anru-history-"));
  let counter = 0;
...
test("history sanitizes messages, evidence URLs, and malformed records", () => withStore((store) => {
  const session = store.save({
    ...
    evidence: { results: [{ doi: "10.1/private", title: "Paper", citation_url: "http://localhost/private" }, { doi: "10.1/public", title: "Public", citation_url: "https://doi.org/10.1/public" }] },
  });
  assert.equal(session.messages.length, 1);
  assert.equal(session.messages[0].text, "answer");
  assert.equal(session.evidence.results[0].citation_url, "");
  assert.equal(session.evidence.results[1].citation_url, "https://doi.org/10.1/public");
}));
...
test("corrupt or oversized history fails closed", () => withStore((store) => {
  ...
  assert.ok(store.recoveryFile);
  assert.equal(fs.readFileSync(store.recoveryFile, "utf8"), "not json");
}));

test("history supports an encrypted-at-rest codec boundary", () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "anru-history-codec-"));
