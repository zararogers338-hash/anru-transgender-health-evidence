...
test("text attachments are extracted in memory and filenames are sanitized", async () => {
  const result = await parseAttachment({ name: "../notes.md", type: "text/markdown", bytes: Buffer.from("Transgender health evidence notes") });
  assert.equal(result.name, "notes.md");
  assert.equal(result.text, "Transgender health evidence notes");
