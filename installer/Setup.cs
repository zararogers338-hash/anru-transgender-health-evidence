...
namespace AnruInstaller
{
    internal static class Program
    {
        private const string PayloadFolder = "app";
        private const long ReserveBytes = 512L * 1024L * 1024L;
        private const string PayloadResource = "Anru.Payload.7z";
        private const string ExtractorResource = "Anru.7za.exe";
        private const string ExtractorDllResource = "Anru.7za.dll";
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                if (options.Silent)
                {
                    using (PayloadLease payload = ExtractPayload(null))
                        Install(payload.AppPath, options.Target, options.CreateDesktopShortcut, options.Launch, null);
                    return 0;
                }

                Application.EnableVisualStyles();
                Application.SetCompatibleTextRenderingDefault(false);
                Application.Run(new InstallerForm(options.Target));
            ...
            catch (Exception error)
            {
                ...
                if (!HasArgument(args, "--silent"))
                    MessageBox.Show(error.Message, "安若安装失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return 1;
            }
        }
        ...
        {
            get { return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "Anru"); }
        }
        ...
        internal static void Install(string source, string target, bool desktopShortcut, bool launch, Action<int, string> progress)
        {
            ...
            if (desktopShortcut) CreateShortcut(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "安若 Anru.lnk"), Path.Combine(target, "Anru.exe"), target);
            ...
            if (launch) Process.Start(new ProcessStartInfo(Path.Combine(target, "Anru.exe")) { WorkingDirectory = target, UseShellExecute = true });
        }

        internal static PayloadLease ExtractPayload(Action<int, string> progress)
        {
            string root = Path.Combine(Path.GetTempPath(), "Anru-Setup-" + Guid.NewGuid().ToString("N"));
            string unpack = Path.Combine(root, "payload");
            Directory.CreateDirectory(unpack);
            try
            {
                if (progress != null) progress(2, "正在准备安装文件");
                string archive = Path.Combine(root, "payload.7z");
                string extractor = Path.Combine(root, "7za.exe");
                ExtractResource(PayloadResource, archive);
                ExtractResource(ExtractorResource, extractor);
                ExtractResource(ExtractorDllResource, Path.Combine(root, "7za.dll"));
                if (progress != null) progress(6, "正在解压安装文件");
                ProcessStartInfo start = new ProcessStartInfo(extractor, "x -y -aoa -o\"" + unpack + "\" \"" + archive + "\"") {
                ...
                {
                    process.WaitForExit();
                    if (process.ExitCode != 0) throw new InvalidDataException("安装载荷解压失败：" + process.ExitCode);
                }
                string appPath = Path.Combine(unpack, PayloadFolder);
                ValidateSource(appPath);
                if (progress != null) progress(20, "安装文件准备完成");
                return new PayloadLease(root, appPath);
            }
            catch
            {
        ...
        private static void ExtractResource(string name, string destination)
        {
            ...
            {
                if (input == null) throw new InvalidDataException("安装器资源缺失：" + name);
        ...
        internal sealed class PayloadLease : IDisposable
        {
            ...
            internal readonly string AppPath;

            internal PayloadLease(string rootPath, string appPath)
            {
                root = rootPath;
                AppPath = appPath;
            }
        ...
        private static void ValidateSource(string source)
        {
            ...
            string[] required = {
                ...
                Path.Combine("resources", "anru", "SKILL.md"),
                Path.Combine("resources", "anru", "data", "anru_evidence.db")
            };
        ...
        private static void VerifyCriticalFiles(string target)
        {
            ...
            foreach (Match match in Regex.Matches(manifest, "\"(executable|database|model)\"\\s*:\\s*\"([0-9a-fA-F]{64})\""))
            ...
            Dictionary<string, string> paths = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) {
                { "executable", Path.Combine(target, "Anru.exe") },
                { "database", Path.Combine(target, "resources", "anru", "data", "anru_evidence.db") }
            };
            string optionalModel = Path.Combine(target, "resources", "anru", "models", "bge-m3", "pytorch_model.bin");
            if (File.Exists(optionalModel)) paths["model"] = optionalModel;
        ...
        {
            string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "安若 Anru");
            Directory.CreateDirectory(folder);
            CreateShortcut(Path.Combine(folder, "安若 Anru.lnk"), Path.Combine(target, "Anru.exe"), target);
        }
        ...
        private static void CreateShortcut(string shortcutPath, string executable, string workingDirectory)
        {
            ...
            shortcut.Description = "Anru 安若 — 跨性别健康循证助手";
        ...
        internal static void WriteLog(string message)
        {
            try
            {
                string file = Path.Combine(Path.GetTempPath(), "Anru-Setup.log");
        ...
        internal sealed class InstallerForm : Form
        {
            ...
            internal InstallerForm(string defaultTarget)
            {
                Text = "Anru 安若安装程序";
            ...
            private void StartInstall(object sender, EventArgs args)
            {
                ...
                Thread worker = new Thread(delegate()
                {
                    try
                    {
                        using (PayloadLease payload = ExtractPayload(delegate(int percent, string file)
                        {
                            BeginInvoke((MethodInvoker)delegate {
                                SetProgress(percent);
                                statusLabel.Text = string.Format(CultureInfo.InvariantCulture, "{0:00}%  /  {1}", percent, file);
                            });
                        }))
                        {
                            Install(payload.AppPath, target, desktopShortcut, false, delegate(int percent, string file)
                            {
                                int totalPercent = 20 + percent * 80 / 100;
                                BeginInvoke((MethodInvoker)delegate {
                                    SetProgress(totalPercent);
                                    statusLabel.Text = totalPercent < 100 ? string.Format(CultureInfo.InvariantCulture, "{0:00}%  /  {1}", totalPercent, file) : "100%  /  安装完成";
                                });
                            });
                        }
                        ExitCode = 0;
                        BeginInvoke((MethodInvoker)delegate
                        {
                            ...
                            installButton.Click -= StartInstall;
                            installButton.Click += delegate { if (launchBox.Checked) Process.Start(new ProcessStartInfo(Path.Combine(target, "Anru.exe")) { WorkingDirectory = target, UseShellExecute = true }); Close(); };
